# Setup Datasync Script Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `scripts/setup_datasync.sh` — a bash script that creates and configures the `datasync` user on remote HAMMA sensors for use by `hamma_download.py`.

**Architecture:** Single bash script that sources `common.sh` for logging/validation, SSHs to each sensor as `pi` through `hamma.dev`, runs setup commands with sudo, and prints verification commands at the end.

**Tech Stack:** Bash, SSH, existing `unified_install/lib/common.sh` library

**Spec:** `docs/superpowers/specs/2026-03-17-setup-datasync-design.md`

**Repo root:** `~/Documents/insync/uah_gdrive/programming/python/hamma_sensor_repos/mjolnir-hamma/`
...
| File | Action | Purpose |
|------|--------|---------|
| `scripts/setup_datasync.sh` | Create | Setup script with arg parsing, SSH helpers, per-sensor setup logic |

---

## Chunk 1: Complete Script and Verification

### Task 1: Create setup_datasync.sh

**Files:**
- Create: `scripts/setup_datasync.sh`
...
Run: `chmod +x scripts/setup_datasync.sh`

- [ ] **Step 3: Verify common.sh sourcing works**

Run: `cd ~/Documents/insync/uah_gdrive/programming/python/hamma_sensor_repos/mjolnir-hamma && bash -n scripts/setup_datasync.sh`

Expected: No syntax errors (exit 0).

- [ ] **Step 4: Test --help output**

Run: `./scripts/setup_datasync.sh --help`

Expected: Usage message printed, exit 0.

- [ ] **Step 5: Test argument validation — missing --key**

Run: `./scripts/setup_datasync.sh 5`

Expected: Error "–key is required", usage printed.

- [ ] **Step 6: Test argument validation — no sensors**

Run: `./scripts/setup_datasync.sh --key ~/.ssh/id_rsa.pub`

Expected: Error "At least one sensor number is required", usage printed.

- [ ] **Step 7: Test --dry-run mode**

Run: `./scripts/setup_datasync.sh --key ~/Documents/rhome/.ssh/id_rsa.pub --dry-run 5`

Expected: Dry-run output showing all SSH commands that would execute, no actual connections made.

- [ ] **Step 8: Test idempotency on mj05 (already has datasync)**

Run from a machine with SSH access to pi@hamma.dev:
```bash
./scripts/setup_datasync.sh --key ~/Documents/rhome/.ssh/id_rsa.pub 5
```

Expected:
- "User datasync already exists on sensor 5, skipping creation"
- All other steps succeed (they are idempotent)
- Verification command printed at end

- [ ] **Step 9: Commit**

```bash
git add scripts/setup_datasync.sh
git commit -m "feat: add setup_datasync.sh for automating sensor datasync user setup"
```
