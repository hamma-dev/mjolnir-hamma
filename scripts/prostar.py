...
"""Monitor, program and control a Morningstar ProStar MPPT charge controller."""
...
CONVERSION_FUNCTIONS = {
    ...
    "V": lambda val: round(val * 96.667 * 2 ** -15, 3),
...
# ProStar MPPT uses same coil addresses as SunSaver
LOAD_STATE_COIL = 0x0001
LOAD_STATE_REGISTER = 0x002E  # Register 46 (0x2E) for ProStar

# Daily log data is stored starting at 0x8000
LOG_BLOCKS = 32
LOG_BLOCK_LENGTH = 16
LOG_START_REGISTER = 0x8000
LOG_VARIABLES = [
    ("hourmeter", "I"),
    ("alarm_daily", "BL"),        # 32-bit for ProStar
    ("vb_min_daily", "V"),
    ("vb_max_daily", "V"),
    ("ahc_daily", "Ah"),
    ("ahl_daily", "Ah"),
    ...
    ("load_fault_daily", "B"),
    ("va_max_daily", "V"),
    ("time_ab_daily", "I"),       # Time in absorption
    ("time_eq_daily", "I"),       # Time in equalization
    ("time_fl_daily", "I"),       # Time in float
    ]
...
def _setup_mppt_client(port=None):
    ...
    mppt_client = pymodbus.client.sync.ModbusSerialClient(
        port=port, **SERIAL_PARAMS_PROSTARMPPT)
    mppt_client.connect()
    return mppt_client
...
def get_log_data(sort=True, filter_bad=True, verbose=False):
    """Get logged data from the ProStar, and optionally sort and filter it."""
    log_data = []
...
def main():
    """Main function for prostar script."""
    ...
    parser_main = argparse.ArgumentParser(
        description=("Monitor, program and control a ProStar MPPT."))
    ...
    if parsed_args.version:
        print(f"ProStar version {__version__}")
        sys.exit(0)
