...
class NoiseDiag(brokkr.pipeline.base.OutputStep):
    ...
    def __init__(self,
        ...
        self.alert_persist_count = alert_persist_count if alert_persist_count >= 1 else 1
        self.reset_after_under = reset_after_under if reset_after_under >= 1 else 1
        if (self.alert_persist_count != alert_persist_count
                or self.reset_after_under != reset_after_under):
            self.logger.warning(
